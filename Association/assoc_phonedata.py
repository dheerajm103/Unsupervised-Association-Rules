...
df = pd.read_csv("myphonedata.csv")                                     # importing dataset
df
df1 = df.drop(["V1","V2","V3"], axis = 1)                               # dropping nominal column
df1
...
plt.xticks(list(range(0, 6)), frequent_itemsets.itemsets[0:6], rotation=90)
plt.xlabel('item-sets')
plt.ylabel('support')
plt.show()
